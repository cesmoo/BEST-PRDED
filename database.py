...
class Database:
    def __init__(self):
        ...
        self.user_sessions = self.db['user_sessions']  # သီးသန့် session per user
    ...
    async def get_user(self, user_id: int) -> dict:
        ...
        if not user:
            user = {
                ...
                "lose_streak": 0,
                "best_streak": 0,
                "session_profit": 0.0,  # ဒီ session ရဲ့ profit
                "profit_target": 30000.0,  # Default profit target
    ...
    async def update_session_profit(self, user_id: int, profit: float):
        """Update session profit and check target"""
        await self.users.update_one(
            {"user_id": user_id},
            {"$inc": {"session_profit": profit}}
        )
        user = await self.get_user(user_id)
        return user
    
    async def reset_session_profit(self, user_id: int):
        """Reset session profit to 0"""
        await self.users.update_one(
            {"user_id": user_id},
            {"$set": {"session_profit": 0.0}}
        )
    ...
    async def settle_bets(self, issue_number: str, actual_size: str, actual_number: int) -> list:
        """Settle bets and return list of settled bets for notification"""
        ...
        for bet in pending:
            ...
            if is_win:
                ...
                await self.users.update_one(
                    {"user_id": user_id},
                    {"$inc": {"total_wins": 1, "total_won": payout, "profit": profit, "win_streak": 1, "lose_streak": -1}}
                )
                ...
                await self.users.update_one(
                    {"user_id": user_id},
                    {"$inc": {"total_losses": 1, "profit": profit, "win_streak": -1, "lose_streak": 1}}
                )
                await self.update_session_profit(user_id, profit)
            
            await self.bets.update_one(
                {"_id": bet["_id"]},
                {"$set": {"actual_size": actual_size, "actual_number": actual_number, "result": result, "payout": payout, "profit": profit, "settled_at": datetime.now()}}
            )
    ...
    async def get_user_bets(self, user_id: int, limit: int = 10) -> list:
        cursor = self.bets.find({"user_id": user_id}).sort("created_at", -1).limit(limit)
    ...
    async def get_sudo_users(self) -> set:
        cursor = self.sudo_users.find({"active": True})
    ...
    async def add_sudo(self, user_id: int, added_by: int) -> bool:
        await self.sudo_users.update_one(
            {"user_id": user_id},
            {"$set": {"user_id": user_id, "active": True, "added_by": added_by, "added_at": datetime.now()}},
            upsert=True
        )
        return True
    ...
    async def get_active_users(self) -> set:
        cursor = self.active_sessions.find({"active": True})
        docs = await cursor.to_list(length=None)
        return {doc["user_id"] for doc in docs}
    
    async def activate_session(self, user_id: int, ai_mode: str):
        await self.active_sessions.update_one(
            {"user_id": user_id},
            {"$set": {"active": True, "activated_at": datetime.now(), "ai_mode": ai_mode}},
            upsert=True
        )
    ...
    # ========== User Sessions (သီးသန့် AI Mode & Settings) ==========
    async def get_user_session(self, user_id: int) -> dict:
        session = await self.user_sessions.find_one({"user_id": user_id, "active": True})
        if not session:
            session = {
                "user_id": user_id,
                "active": False,
                "ai_mode": "pattern",
                "bet_sequence": [100, 300, 900, 2700, 8100],
                "profit_target": 30000.0,
                "created_at": datetime.now()
            }
    ...
    async def get_leaderboard(self, limit: int = 10) -> list:
        cursor = self.users.find().sort("balance", -1).limit(limit)
        return await cursor.to_list(length=limit)
    
    # ========== History ==========
    async def get_history(self, limit: int = 5000) -> list:
        cursor = self.history.find().sort("issue_number", -1).limit(limit)
        return await cursor.to_list(length=limit)
    
    async def add_history(self, issue_number: str, number: int, size: str):
        await self.history.update_one(
            {"issue_number": issue_number},
            {"$setOnInsert": {"number": number, "size": size}},
            upsert=True
        )
